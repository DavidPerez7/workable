...
public class PostulacionMapperImpl implements PostulacionMapper {
  ...
  Short estadoId = java.util.Objects.requireNonNull(postulacionDto.getEstadoId(), "El id de estado no puede ser nulo");
  PostulacionEstado estado = estadoRepository.findById(estadoId)
    .orElseThrow(() -> new EntityNotFoundException("Estado no encontrado"));
    postulacion.setPostulacionEstado(estado);

  Integer ofertaId = java.util.Objects.requireNonNull(postulacionDto.getOfertaId(), "El id de oferta no puede ser nulo");
  Oferta oferta = ofertaRepository.findById(ofertaId)
    .orElseThrow(() -> new EntityNotFoundException("Oferta no encontrada"));
    postulacion.setOferta(oferta);

  Integer aspiranteId = java.util.Objects.requireNonNull(postulacionDto.getAspiranteId(), "El id de aspirante no puede ser nulo");
  UsrAspirante aspirante = aspiranteRepository.findById(aspiranteId)
